...
- **404 (Not Found) for an item icon (e.g. `clothing-shirt.svg`)**  
  The browser is requesting an image from the **D&D 5e system** (e.g. `systems/dnd5e/icons/svg/items/...`). The Garou module uses `feature.svg` for its items; it does not reference `clothing-shirt.svg`. The 404 usually means either (1) another item or actor in the world has its image set to a path that does not exist in your dnd5e install, or (2) the dnd5e system uses a default icon for an equipment type that is missing in your version.  
  **Fix:** (1) Update the D&D 5e system. (2) Find the document with the broken icon (Actors, Items, or compendia — look for a broken image) and set **Image** to a valid path, e.g. `systems/dnd5e/icons/svg/items/feature.svg` or the module fallback `modules/garou/assets/icons/items/feature.svg`. The Garou module provides a fallback icon at `modules/garou/assets/icons/items/feature.svg`.
